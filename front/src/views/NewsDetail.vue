...
import { getApiLocale, getNewsItem, submitLead } from '@/api';
...
const loadNews = async () => {
  try {
    news.value = await getNewsItem(newsId, getApiLocale(locale.value));
  } catch (error) {
...
const sendLead = async () => {
  ...
  try {
    await submitLead({
      ...
      message: `Lead from news page: ${news.value.title}`,
